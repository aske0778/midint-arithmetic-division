...
powDiff(uint32_t* USh, uint32_t* VSh, uint32_t VReg[Q], uint32_t RReg[Q], uint32_t h, uint32_t l, uint32_t g) {
...
refine(uint32_t* USh, uint32_t* VSh, uint32_t VReg[Q], uint32_t TReg[Q], uint16_t h, uint16_t k, uint8_t l, uint32_t RReg[Q]) {
    ...
    while (h - k > l) {
        ...
        uint32_t s = max(0, k - 2 * l + 1 - g);
        shift<M, Q>(-s, VReg, USh, TReg);
        step<M, Q>(USh, VSh, k + l + n - s + g, TReg, RReg, n, l, g);
        shift<M, Q>(-1, RReg, USh, RReg);
        l = l + n - 1;
    }
    shift<M, Q>(-g, RReg, USh, RReg);
}

template<uint32_t M, uint32_t Q>
__device__ inline void
shinv(uint32_t* USh, uint32_t* VSh, uint32_t VReg[Q], uint32_t TReg[Q], uint32_t h, uint32_t RReg[Q]) {
    ...
    if (h - k <= l) {
        ...
        refine<Q>(USh, VSh, VReg, TReg, h, k, l, RReg[Q]);
    }
}
...
__global__ void divShinv(uint32_t* u, uint32_t* v, uint32_t* quo, uint32_t* rem)
{
    ...
    //sub<>(USh, VSh, UReg, RReg2, RReg2); 

    if (!lt<Q>(RReg2, VReg, USh)) {
        add1<Q>(RReg1, USh);   
        //sub<>(USh, VSh, RReg2, VReg, RReg2);  
    }
