...
let badd1u16 [m] (us : [(4*m)]u16) : [(4*m)]u16 =
...
def lt [m] (u: [(4*m)]u16) (v: [(4*m)]u16) : bool =
...
--def ltbpow [m] (u: [(4*m)]u16) (bpow: i64) : bool =
--  let v = zeroAndSet 1 bpow (4 * m) :> [(4*m)]u16
--  let res = map2 (\ x y -> (x < y, x == y) ) u v
--  |> reduce (\ (l1, e1) (l2, e2) -> (l2 || (e2 && l1), e1 && e2) ) (false, true)
--  in res.0
 
def shift [m] (shft : i64) (u : [(4*m)]u16) : ([(4*m)]u16) =
    map (\ idx -> let offset = idx - shft 
           in if offset < ((4*m)) && offset >= 0 then u[offset] else 0) (iota ((4*m)))

def eqBpow [m] (u : [(4*m)]u16) (b : i64) : bool =
    let bpow = zeroAndSet 1 b ((4*m))
    in reduce (\ x y -> (x == y && x != false)) true (map2 (==) u bpow)

def ltBpow [m] (u: [(4*m)]u16) (b: i64) : bool =
    let bpow = zeroAndSet 1 b ((4*m))
    in lt u bpow

def quo_single [m] (bpow : i64) (d :[(4*m)]u16) (n : i64) : ([(4*m)]u16) =
    let ret = replicate n 0u16 :> [(4*m)]u16
