...
--
def multmod [m] (us: [(4*m)]u16) (vs: [(4*m)]u16) (d: i64) : [(4*m)]u16 = 
    let res = convMulV2 us vs
    in tabulate ((4*m)) (\i -> if i >= d then 0u16 else res[i])

--
-- Calculates B^h-v*w
--
def powDiff [m] (vs: [(4*m)]u16) (ws: [(4*m)]u16) (h: i64) (l: i64) : (u32, [(4*m)]u16) =
    let precV = prec vs
    let precW = prec ws
    let L = precW + precV - l + 1

    in if (precV == 0 || precW == 0) then
        let ret = zeroAndSet 1u16 h (4*m)
        let ret = ret :> [(4*m)]u16
        in (1, ret)
    else if (L >= h) then
        let ret = convMulV2 vs ws
        in if ltBpow ret h then
            let bpow = zeroAndSet 1 h ((4*m))
...
--
def step [m] (vs: [(4*m)]u16) (ws: [(4*m)]u16) (h: i64) (l: i64) (n: i64) : [(4*m)]u16 =
...
--
def refine [m] (vs: [(4*m)]u16) (ws: [(4*m)]u16) (h: i64) (k: i64) (l: i64) : [(4*m)]u16 =
    let ws = shift 2 ws
...
--
def shinv [m] (vs: [(4*m)]u16) (h: i64) (k: i64) : [(4*m)]u16 =
    if k == 0 then
        quo_single h (vs) (4*m) :> [(4*m)]u16 
    else if k >= h && !(eqBpow vs h) then
        vs
    else if k == h - 1 && vs[k] > u16.highest / 2 then
        zeroAndSet 1 0 (4*m) :> [(4*m)]u16
    else if eqBpow vs k then
        zeroAndSet 1 (h - k) (4*m) :> [(4*m)]u16
    else 
        let l = i64.min k 2
...
--
def div [m] (us: [(4*m)]u16) (vs: [(4*m)]u16) : ([(4*m)]u16, [(4*m)]u16) =
    let h = prec us
    let k = (prec vs) - 1
    ...
    let quo = 
        let m = m * 2
        let quo_padded = ((trace (shinv vs h k)) ++ (replicate (4*(m/2)) 0u16)) :> [(4 * m)]u16
        let us_padded = (us ++ (replicate (4*(m/2)) 0u16)) :> [(4 * m)]u16
        let mul_res = convMulV2 quo_padded us_padded
        let mul_shifted = shift (-h) mul_res
        let res = take (4*(m/2)) mul_shifted
        in res
    let quo = quo :> [(4 * m)]u16
...
-- the shifted inverse and classical multiplication
--
-- try mulv1 and baddV2
-- def quo [m] (us: [(4*m)]u16) (vs: [(4*m)]u16) : [(4*m)]u16 =
--     let h = prec us
--     let k = (prec vs) - 1
-- 
--     let (us, vs, h, k) =
...
entry test_div  [m] (us: [m]u16) (vs: [m]u16) : ([m]u16, [m]u16) =
    let mdiv4 = m / 4
    let us = us :> [(4*mdiv4)]u16
    let vs = vs :> [(4*mdiv4)]u16
...
entry bench_div [n][m] (us: [n][m]u16) (vs: [n][m]u16) : ([n][(4*m)]u16, [n][(4*m)]u16) =
    #[unsafe]
    let mdiv4 = m / 4
    let us = us :> [n][(4*mdiv4)]u16
    let vs = vs :> [n][(4*mdiv4)]u16
    let ret = map2 div us vs |> unzip :> ([n][(4*m)]u16, [n][(4*m)]u16)
    in  ret
