...
def set [n] (u : *[n]u64) (d : u64) (idx : i32) : [n]u64 = 
    let u[idx] =  d in u

-- zero bigint array and set given index to d
def zeroAndSet (d : u64) (idx : i64) (m : i64) : []u64 = 
    tabulate m (\i -> if i == idx then d else 0u64)
...
-- less than taken from thorbjørn, musch cleaner than mine 
-- source : https://github.com/tossenxD/big-int/blob/main/futhark/helper.fut
def shift [m] [ipb] (shft : i64) (u : [ipb * (4 * m)]u64) : ([ipb * (4 * m)]u64) =
    map (\ idx -> let offset = idx - shft 
           in if offset < m && offset >= 0 then u[offset] else 0) (iota (ipb * (4 * m)))
...
def eqBpow [m] (u : [m]u64) (b : i64) : bool =
    let bpow = zeroAndSet 1 (b) m
    in reduce (\ x y -> (x == y && x != false)) true (map2 (==) u bpow)

def ltBpow [m] (u: [m]u64) (b: i64) : bool =
    let bpow = zeroAndSet 1 b m
    in lt u bpow
