...
void verifyResultsGCD(uint32_t num_instances, instance_t  *instances, const uint32_t val) {
  uint32_t buffer[BITS/32];
  for(uint32_t i=0; i<num_instances; i++) {
      gmpGCDOnce<uint32_t, BITS/32>(&instances[i].a._limbs[0], val, &buffer[0]);
      for(uint32_t j=0; j<BITS/32; j++) {
           if ( buffer[j] != instances[i].sum._limbs[j] ) {
              printf( "INVALID RESULT at instance: %u, local index %u: %u vs %u\n"
                    , i, j, buffer[j], instances[i].sum._limbs[j]
...
// launch with 32 threads per instance, 128 threads (4 instances) per block
for(int i = 0; i < GPU_RUNS_CMUL; i++)
kernel_gcd<<<(num_instances+ipb-1)/ipb, cuda_block>>>(report, gpuInstances, num_instances, val);
cudaDeviceSynchronize();
...
//printf("Verifying the results ...\n");
//verify_results(instances, num_instances);
verifyResultsGCD(num_instances, instances, val);
}
...
int main(int argc, char * argv[]) {
  ...
  // add and mul bechmarks
  {
    ...
    runGCD(num_instances, 128, report, gpuInstances, instances, val);
